...
import { LOCATION_LABELS, type BlueprintLocation } from '@/lib/blueprint'

/**
 * Simplified program picker for beginners (0-1 years experience).
 * Skips goal selection (everyone starts with muscle building) and limits
 * to 3-4 days for focused progress. Much cleaner UX for first-timers.
 */
export default function BeginnerPicker({
  firstName,
  onDone,
}: {
  firstName?: string
  ...
  const dayChoices = [3, 4] // Beginners should start with 3-4, not more

  const start = async () => {
    if (!location || !days) return
    setSubmitting(true)
    setError('')
    try {
      // For beginners, always use 'muscle' goal (optimal for starting out)
      const res = await fetch('/api/studio/blueprint/assign', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ goal: 'muscle', location, days }),
      })
  ...
  return (
    <div className="max-w-xl mx-auto">
      ...
      <div className="text-center mb-6">
        <span className="inline-block text-[10px] font-display font-bold px-2.5 py-1 rounded bg-emerald-500/15 text-emerald-400 uppercase tracking-wide">
          Welcome to training
        </span>
        <h1 className="font-display font-extrabold text-2xl text-white tracking-tight mt-3">
          {firstName ? `Nice to meet you, ${firstName}` : "Let's get started"}
        </h1>
        <p className="text-white/40 text-sm font-body mt-1">
          Two quick questions and you're training. No overthinking — we'll handle the programming.
        </p>
      </div>

      {/* Progress dots */}
      <div className="flex items-center justify-center gap-2 mb-6">
        {['Where', 'How often'].map((label, i) => (
          <div key={label} className="flex items-center gap-2">
            <div
              className={`w-2.5 h-2.5 rounded-full transition-colors duration-200 ${
                i < step ? 'bg-emerald-500' : i === step ? 'bg-brand-blue' : 'bg-white/15'
              }`}
            />
          </div>
        ))}
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 1 && (
            <motion.div key="days" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.2 }}>
              <h2 className="font-display font-bold text-white text-lg mb-4">How many days can you train?</h2>
              ...
              {days && (
                <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="rounded-xl bg-white/[0.03] border border-white/[0.08] p-4 mb-5">
                  ...
                  <p className="text-white font-display font-bold text-base mt-1">
                    {days === 3 ? 'Full Body 3x/week' : 'Upper/Lower 4x/week'}
                  </p>
                  <p className="text-white/40 text-xs font-body mt-1">Optimized for beginners · Build strength + muscle</p>
                </motion.div>
              )}

  ...
  return (
    <>
      <button
        onClick={() => setOpen((v) => !v)}
        title="Send feedback"
        className="fixed bottom-24 md:bottom-6 left-4 md:left-6 z-50 px-4 h-11 rounded-full bg-white/[0.08] backdrop-blur border border-white/[0.12] text-white/80 text-xs font-display font-bold uppercase tracking-wide flex items-center gap-2 hover:bg-white/[0.14] transition-colors duration-200"
      >
      ...
      {open && (
        <div className="fixed bottom-[152px] md:bottom-20 left-4 md:left-6 z-50 w-[calc(100vw-2rem)] max-w-[320px] rounded-card border border-white/[0.12] bg-surface-overlay shadow-[0_20px_60px_rgba(0,0,0,0.5)] p-4">
          {done ? (

  ...
  return (
    <div className="p-6 lg:p-10 max-w-[1400px] mx-auto">
      ...
      <div className="flex gap-2 mb-6">
        <button onClick={() => setActiveView('clients')} className={`px-5 py-2.5 rounded-lg text-xs font-display font-bold uppercase tracking-wide transition-all duration-200 ${activeView === 'clients' ? 'bg-brand-orange text-white' : 'bg-white/[0.04] text-white/40 hover:text-white/70'}`}>
          Client Messages
        </button>
        <button onClick={() => setActiveView('chaedyn')} className={`px-5 py-2.5 rounded-lg text-xs font-display font-bold uppercase tracking-wide transition-all duration-200 flex items-center gap-2 ${activeView === 'chaedyn' ? 'bg-brand-orange text-white' : 'bg-white/[0.04] text-white/40 hover:text-white/70'}`}>
            ...
            <div className="lg:col-span-4 border-r border-white/[0.06]">
              <div className="p-4 border-b border-white/[0.06]">
                <div className="relative">
                  ...
                  <input value={search} onChange={(e) => setSearch(e.target.value)} type="text" placeholder="Search clients..." className="w-full bg-white/[0.04] border border-white/[0.06] rounded-lg pl-10 pr-4 py-2.5 text-sm text-white/70 font-body placeholder:text-white/20 focus:outline-none focus:border-brand-orange/30" />
                </div>
              </div>
                ...
                <>
                    ...
                    {messages.length === 0 ? (
                      ...
                      messages.map((msg) => (
                        <div key={msg.id} className={`flex ${msg.from_trainer ? 'justify-end' : 'justify-start'}`}>
                          <div className={`max-w-[75%] px-4 py-3 rounded-2xl ${msg.from_trainer ? 'bg-brand-blue text-white rounded-br-md' : 'bg-white/[0.06] text-white/80 rounded-bl-md'}`}>
                  ...
                  <div className="px-6 py-4 border-t border-white/[0.06]">
                    <div className="flex items-center gap-3">
                      <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); send() } }} type="text" placeholder="Type a message..." className="flex-1 bg-white/[0.04] border border-white/[0.06] rounded-lg px-4 py-2.5 text-sm text-white/70 font-body placeholder:text-white/20 focus:outline-none focus:border-brand-orange/30" />
                      <button onClick={send} disabled={!input.trim() || sending} className="w-10 h-10 rounded-lg bg-brand-orange flex items-center justify-center hover:bg-brand-orangedark active:scale-95 transition-all duration-200 disabled:opacity-30">
